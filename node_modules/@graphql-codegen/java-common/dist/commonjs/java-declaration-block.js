...
${visitor_plugin_common_1.indentMultiline(method.implementation)}
    ...
    get string() {
        ...
        const members = this._members.length ? visitor_plugin_common_1.indentMultiline(stripIndent(this._members.map(member => this.printMember(member) + ';').join('\n'))) : null;
        const methods = this._methods.length ? visitor_plugin_common_1.indentMultiline(stripIndent(this._methods.map(method => this.printMethod(method)).join('\n\n'))) : null;
